...
export function SouqlyPageContent() {
  ...
  return (
    <section className="relative overflow-hidden">
      {/* Full-bleed backdrop: the same shooting-star field as the homepage
          hero, spanning the whole section (not just the text column) so the
          page reads as one animated scene edge to edge. */}
      <HeroBackground />
      <motion.div
        aria-hidden
        className="pointer-events-none absolute -top-16 left-1/2 -z-10 h-[34rem] w-[34rem] -translate-x-1/2 rounded-full"
        style={{
          background: 'radial-gradient(circle, var(--grad-a) 0%, transparent 70%)',
        }}
        initial={{ opacity: 0 }}
        animate={reduce ? { opacity: 0.16 } : { opacity: [0.1, 0.22, 0.1], scale: [1, 1.08, 1] }}
        transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        aria-hidden
        className="pointer-events-none absolute top-1/2 left-[15%] -z-10 h-[22rem] w-[22rem] -translate-y-1/2 rounded-full"
        style={{
          background: 'radial-gradient(circle, var(--grad-b) 0%, transparent 72%)',
        }}
        initial={{ opacity: 0 }}
        animate={reduce ? { opacity: 0.1 } : { opacity: [0.06, 0.16, 0.06], scale: [1, 1.12, 1] }}
        transition={{ duration: 7, delay: 1, repeat: Infinity, ease: 'easeInOut' }}
      />
      <motion.div
        aria-hidden
        className="pointer-events-none absolute right-[10%] bottom-0 -z-10 h-[20rem] w-[20rem] rounded-full"
        style={{
          background: 'radial-gradient(circle, var(--grad-a) 0%, transparent 72%)',
        }}
        initial={{ opacity: 0 }}
        animate={reduce ? { opacity: 0.08 } : { opacity: [0.05, 0.14, 0.05], scale: [1, 1.15, 1] }}
        transition={{ duration: 8, delay: 2, repeat: Infinity, ease: 'easeInOut' }}
      />

      {/* Twinkling sparkle field, spread across the full section */}
      {!reduce &&
        SPARKLES.map((s, i) => (
          <motion.span
            key={i}
        ...
        >
          ...
          <div className="group relative inline-block cursor-default select-none">
            ...
            <TiltCard disableTilt={reduce ?? undefined}>
              <motion.div
                className="border-border bg-card relative flex h-[52px] items-center gap-2.5 overflow-hidden rounded-2xl border px-3 opacity-70 shadow-lg grayscale sm:h-[58px] sm:px-3.5"
                initial={reduce ? false : { opacity: 0, scale: 0.6 }}
            ...
            <motion.span
              className="bg-primary text-primary-foreground absolute -top-2.5 -right-2.5 flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold tracking-wide shadow-md"
          ...
          {LINKS.map((link, i) => (
            ...
            >
              <Link href={link.href} className="block">
                <motion.div
                  className="facet-cut group border-border bg-card flex items-start gap-4 rounded-xl border p-5 sm:p-6"
                  whileHover={
                    reduce ? undefined : { scale: 1.015, borderColor: 'var(--primary)', y: -2 }
                  }
                  whileTap={reduce ? undefined : { scale: 0.985 }}
                  transition={{ type: 'spring', stiffness: 340, damping: 22 }}
                >
                  <motion.span
                    className="glow bg-primary/10 text-primary flex size-11 shrink-0 items-center justify-center rounded-lg"
                  ...
                  <span>
                    <span className="font-heading text-foreground group-hover:text-primary block text-base font-semibold">
                      {link.title}
                    </span>
